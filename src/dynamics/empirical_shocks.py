...
* The primitive residuals are weakly but genuinely dependent, with integrated
  correlation times of roughly 8 blocks (execution), 31 (data) and 23 (state)
  after price adjustment and de-seasonalisation, and they co-move strongly --
  execution/data 0.78, execution/state 0.72, data/state 0.61. Sampling them
...
# Recalibrated on the 60-day joint panel over complete 57,600-block synthetic
# paths. Among 400, 800, 1,600, 3,200 and 6,400, the 3,200-block candidate best
# reproduces the source ACF shape, integrated correlation time and clustered
# upper tail; lagged cross-resource errors remain below 0.002 for every choice.
DEFAULT_BLOCK_LENGTH = 3200
...
class ShockPanel:
    """Contiguous joint residual panel in logs, with anchor-preserving shocks.

    Primitive activity shocks have arithmetic mean one in levels. The access
    residual has parent-BAL-weighted mean one, so its correlation with parent
    activity does not shift the runtime-BAL anchor.
    """
...
def _mean_one_log_center(values: pd.DataFrame) -> pd.DataFrame:
    """Shift log residuals so each multiplicative shock has arithmetic mean one.

    Median-centering a log residual makes its typical value one, but a dispersed
    log shock then has a level mean above one. The demand anchors are mean block
    quantities, so retaining that Jensen gap would silently raise average latent
    demand relative to the anchor, most strongly for the volatile state series.
    """

    centred = values - values.median()
    log_level_mean = np.log(np.exp(centred).mean(axis=0))
    return centred - log_level_mean


def _parent_bal_weight_center(
    access_log_residual: pd.Series,
    parent_bal: np.ndarray | pd.Series,
) -> pd.Series:
    """Center an access residual to preserve mean parent-generated BAL.

    BAL equals the access-composition factor times the BAL generated by parent
    execution and state activity. An unweighted mean-one access factor is not
    anchor preserving when those terms are correlated. This normalization
    imposes ``sum(a * parent_bal) / sum(parent_bal) == 1`` in levels.
    """

    access = access_log_residual.astype(float)
    weights = np.asarray(parent_bal, dtype=float)
    if len(access) != len(weights):
        raise ValueError("access residual and parent BAL weights must have equal length")
    if not np.isfinite(weights).all() or np.any(weights <= 0):
        raise ValueError("parent BAL weights must be finite and positive")

    centred = access - access.median()
    weighted_level_mean = np.average(np.exp(centred.to_numpy()), weights=weights)
    if not np.isfinite(weighted_level_mean) or weighted_level_mean <= 0:
        raise ValueError("weighted access level mean must be finite and positive")
    return centred - np.log(weighted_level_mean)
...
def build_shock_panel(
    ...
    panel. The joint panel is restricted to blocks observed in both inputs so
    that the measured access--primitive dependence is preserved.
    """
    ...
    primitive_columns = ["s_execution", "s_data", "s_state"]
    centred = _mean_one_log_center(residuals[primitive_columns])

    # At the reference parent prices and rho_A=1, arithmetic-mean-one primitive
    # shocks reproduce the execution and state activity anchors. Weighting the
    # access residual by the BAL those activities generate then preserves the
    # runtime-BAL anchor even though access composition is correlated with the
    # primitive shocks. The formula also remains well defined for a non-unit
    # rho_A supplied to this constructor.
    q_state_0 = float(demand["q_state_per_block"])
    parent_bal = (
        w_execution
        * q_execution_0
        * np.exp(centred["s_execution"].to_numpy()) ** rho_A
        + w_state * q_state_0 * np.exp(centred["s_state"].to_numpy())
    )
    centred["a_access"] = _parent_bal_weight_center(
        residuals["a_access"], parent_bal
    )
    centred = centred[list(SHOCK_COLUMNS)]
